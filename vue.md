...
### 12.2 自定义过滤器
#### 过滤器本质上是一个函数，这个函数接收一个值，将其处理后的返回。用管道字符| 分隔，其后跟一个或多个参数，
#### 用空格分开。参数如带引号，为纯字符串静态值，不用引号，则为表达式，动态计算。过滤器可以多个同时使用，用|分开。
...
### 13.4 selected
### 13.5 动态绑定
### 13.6 lazy 将同步改变到change事件中发生。也就是离开输入框，数据同步更新
### 13.7 number 数字
### 13.8 debounce 延迟同步输入框的值。延迟某时间输入框数据才同步更新